  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 flex-wrap no-print mb-4">
        ...
        {newReports && <span className="text-[11px] text-text-3">{range.label} — Reopens Won / Recovered by Others</span>}
      </div>
      <div className="border border-border rounded-xl overflow-x-auto">
        <div
          className={cn(
            'grid px-4 py-2.5 bg-surface-2 text-[11px] font-bold uppercase tracking-wider text-text-3',
            newReports
              ? 'grid-cols-[1fr_repeat(5,80px)_100px_repeat(2,110px)_130px] min-w-[980px]'
              : 'grid-cols-[1fr_repeat(5,80px)_100px] min-w-[640px]',
          )}
        >
        ...
        {perAM.map(({ am, total, open, won, paid, lost, cross }) => {
          ...
          return (
            <div
              key={am}
              className={cn(
                'grid px-4 py-2.5 border-t border-border hover:bg-surface-2/60 text-[13px]',
                newReports
                  ? 'grid-cols-[1fr_repeat(5,80px)_100px_repeat(2,110px)_130px] min-w-[980px]'
                  : 'grid-cols-[1fr_repeat(5,80px)_100px] min-w-[640px]',
              )}
    ...
    {newReports && <RequalificationRulesPanel from={range.startISO} to={range.endISO} />}
    </div>
  );
}
