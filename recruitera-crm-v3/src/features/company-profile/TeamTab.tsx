...
import { useContacts } from '@/hooks/useAccountDetail';
    ...
    <div className="bg-surface border border-border rounded-2xl p-6 shadow-sh1">
      ...
      <div className="flex flex-col gap-2.5">
        {contacts.map((c) => (
          <div
            key={c.id}
            className="grid grid-cols-[40px_1.4fr_1.4fr_1.2fr_24px] gap-3.5 items-center px-3.5 py-3 bg-surface-2 border border-border rounded-xl"
          >
            ...
            <button title="Edit" className="w-7 h-7 rounded-md text-text-3 hover:bg-surface hover:text-text flex items-center justify-center">✎</button>
          </div>
        ))}
      </div>
    </div>
