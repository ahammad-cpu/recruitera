...
/**
 * Mark an account as lost — real sales attempt at SQL / Demo / Proposal
 * that didn't close. For Lead / MQL "not-a-real-prospect" rejections, use
 * DisqualifyModal instead. Both write to accounts.stage='lost' via the
 * lose_account RPC; distinguishable in reports by reason_code.
 */
      ...
      <div className="bg-surface rounded-2xl shadow-sh3 w-full max-w-xl overflow-hidden flex flex-col my-8 border border-border">
        <header className="flex items-start gap-4 px-6 py-5 bg-gradient-to-r from-bad-bg via-bad-bg/60 to-surface border-b border-border relative">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[10px] font-black tracking-[0.18em] uppercase text-bad">Mark account as lost</div>
            ...
            <div className="text-[12.5px] text-text-2 mt-1">
              Capture why this account didn&apos;t close — feeds the lost-reason report.
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <footer className="px-6 py-4 border-t border-border bg-surface-2/50 flex items-center justify-end gap-2">
          ...
          >
            <X size={14} strokeWidth={3} /> {lose.isPending ? 'Marking…' : 'Mark as lost'}
          </button>
        </footer>
      </div>
    </div>
