...
-- NOTE: tasks are rows in `activities` with type='task'. The FK targets activities.
...
begin
  ...
  -- Tasks are activities with type='task'. Uses `assigned_to` (not owner_id) and no `kind` column.
  insert into activities (account_id, type, title, assigned_to, priority, author_id)
  select p_account_id,
         'task',
         'Follow up: re-engage ' || coalesce(a.name, '(unnamed)'),
         a.owner_id, 'medium', auth.uid()
...
Deno.serve(async () => {
  ...
  for (const rule of rules ?? []) {
    ...
    for (const acct of matches ?? []) {
      ...
      // Tasks are stored as activities with type='task'; assignee via assigned_to.
      const { data: task, error: taskErr } = await sb.from("activities").insert({
        ...
        text: `Auto-generated by requalification rule: ${rule.name}`,
      }).select("id").single();
